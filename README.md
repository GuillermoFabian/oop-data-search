...
Using OOP with Python to search for and explore close approaches of near-Earth objects (NEOs), using data from NASA/JPL's Center for Near Earth Object Studies.

## Overview

Using command-line tool to inspect and query a dataset of NEOs and their close approaches to Earth.

Reads data from a CSV file and a JSON file, convert that data into structured Python objects, perform filtering operations on the data, limit the size of the result set, and write the results to a file in a structured format, such as CSV or JSON.

It is possible to inspect the properties of the near-Earth objects in the data set and query the data set of close approaches to Earth using any combination of the following filters:
...
Run `python3 main.py --help` for an explanation of how to invoke the script.
